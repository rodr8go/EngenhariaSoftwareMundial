...
public class PatrociniosPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        removeBtn.addActionListener(e -> {
            ...
            boolean temContratos = DataStore.getInstance().getContratos().stream()
                .anyMatch(c -> c.getPatrocinador() == p);
            if (temContratos) {
                JOptionPane.showMessageDialog(this,
                    "Não é possível remover \"" + p.getNome() + "\" porque tem contratos associados.",
                    "Remoção bloqueada", JOptionPane.WARNING_MESSAGE);
                return;
            }
    ...
    private void showContratoDialog() {
        ...
        if (ds.getJogosTorneio().isEmpty()) {
            JOptionPane.showMessageDialog(this,
                "Não existem jogos disponíveis. É obrigatório associar um jogo ao contrato.",
                "Sem jogos", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (jogoCombo.getSelectedIndex() < 0) {
                JOptionPane.showMessageDialog(dlg, "Selecione um jogo associado.", "Campo obrigatório", JOptionPane.WARNING_MESSAGE);
                return;
            }
            try {
                ...
                if (jogoIdx >= 0) {
                    ...
                    if (jogoIdx < jogos.size()) jogoSel = jogos.get(jogoIdx);
                }
